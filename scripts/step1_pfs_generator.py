...
def load_hybrid_profiles(iso):
    """Load pre-computed hybrid co-located profiles from NPZ.

    Returns dict mapping hybrid type → (8760,) numpy array.
    Profiles are normalized (sum=1.0) energy-share space, same as
    solar/wind profiles from get_supply_profiles().

    Source: data/hybrid_profiles/{ISO}_hybrid_profiles.npz
    Generated by scripts/generate_hybrid_profiles.py.
    """
    hybrid_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                              'data', 'hybrid_profiles')
    path = os.path.join(hybrid_dir, f'{iso}_hybrid_profiles.npz')

    if not os.path.exists(path):
        raise FileNotFoundError(
            f"Hybrid profiles not found: {path}\n"
            f"Run: python scripts/generate_hybrid_profiles.py")

    npz = np.load(path)
    profiles = {}
    for ht in HYBRID_TYPES:
        if ht not in npz:
            raise KeyError(f"Missing hybrid type '{ht}' in {path}")
        arr = np.asarray(npz[ht][:H], dtype=np.float64)
        np.maximum(arr, 0.0, out=arr)
        if len(arr) < H:
            arr = np.pad(arr, (0, H - len(arr)))
        profiles[ht] = arr
...
def prepare_numpy_profiles(iso, demand_norm, supply_profiles,
                           include_hybrids=False, hybrid_profiles=None):
    ...
    (n_resources, 8760). n_resources depends on ISO and include_hybrids:
      - Interior ISOs:  4D base or 8D with hybrids
      - Offshore ISOs:  5D base or 9D with hybrids
      - CAISO:          6D base or 10D with hybrids
    ...
    # Merge supply_profiles + hybrid_profiles into one dict
    all_profiles = dict(supply_profiles)
    if include_hybrids:
        if hybrid_profiles is None:
            raise ValueError("include_hybrids=True but hybrid_profiles not provided")
        all_profiles.update(hybrid_profiles)

    supply_matrix = np.stack([
        np.asarray(all_profiles[rt][:H], dtype=np.float64)
        for rt in rtypes
