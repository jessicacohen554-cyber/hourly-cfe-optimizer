...
Pipeline position:
  Step 1 (PFS) → Step 2 (EF) → Step 3 (Cost) → Step 4 (Postprocess)
                                                      ↓
                                          step5b_compute_lmp_prices.py  ← THIS
                                                      ↓
                              data/step5-post-processing/lmp/{ISO}_lmp.parquet   (per-ISO output)
                              data/step5-post-processing/lmp/lmp_summary.json

Usage:
  python step5b_compute_lmp_prices.py --iso PJM                    # PJM only, all thresholds
  python step5b_compute_lmp_prices.py --iso PJM --test             # PJM test: 2025/50%/95%
  python step5b_compute_lmp_prices.py                              # PJM only (default)
  python step5b_compute_lmp_prices.py --iso PJM --fuel-level M     # Medium fuel only
"""
