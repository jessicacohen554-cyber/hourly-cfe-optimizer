...
# Hybrid co-located resource types (solar/wind + battery)
...
def get_resource_cols(iso, include_hybrids=False):
    ...
    Args:
        iso: ISO region name
        include_hybrids: If True, append hybrid co-located types to the list
    """
    if iso == 'CAISO':
        return RESOURCE_COLS_CAISO_HYBRID if include_hybrids else RESOURCE_COLS_CAISO
    elif iso in OFFSHORE_ISOS:
        return RESOURCE_COLS_OFFSHORE_HYBRID if include_hybrids else RESOURCE_COLS_OFFSHORE
    else:
        return RESOURCE_COLS_BASE_HYBRID if include_hybrids else RESOURCE_COLS_BASE
...
PEAK_CAPACITY_CREDITS = {
    ...
    'offshore_wind': 0.25,
    # Hybrid co-located: higher than standalone — battery shifts generation to peak hours
...
# ============================================================================
# HYBRID CO-LOCATED RESOURCE PARAMETERS
# ============================================================================
# Solar+Battery and Wind+Battery co-located hybrids. Pre-computed 8760 profiles
# generated by scripts/generate_hybrid_profiles.py (stored in data/hybrid_profiles/).
#
# LCOE model: component-additive.
#   hybrid_lcoe = adjusted_renewable_lcoe + battery_lcos × (1 - ITC_STORAGE_RATE)
# Solar hybrids: renewable LCOE divided by DC:AC ratio (overbuild increases CF).
# Wind hybrids: no DC:AC adjustment (wind turbines are AC machines).
#
# Toggles: renewable component follows Renewable Gen toggle (ren_lvl),
#          battery component follows Battery toggle (batt_lvl). No new toggles.

# DC:AC ratios for solar hybrids — from generate_hybrid_profiles.py validation.
# Solar hybrid overbuilds DC capacity behind AC interconnection point.
# CAISO lower (existing high-CF sites need less overbuild).
# Wind hybrids have no DC:AC overbuild (battery sized as % of nameplate, not DC:AC).
HYBRID_DC_AC_RATIOS = {
...
# IRA Section 48 ITC for co-located battery storage
ITC_STORAGE_RATE = 0.30

# ---- Hybrid battery LCOS ($/MWh discharged, BEFORE ITC) ----
# Levelized cost of storage for the battery component of hybrid resources.
# Same CAPEX components as standalone batteries (NREL ATB 2024):
#   4hr: L=$240, M=$295, H=$375 per kWh. 8hr: L=$205, M=$253, H=$323 per kWh.
# Same financials: CRF=0.1019, FOM_rate=2.5% → annualization=0.1269.
# RTE=85%, 365 cycles/yr (1 full cycle/day — validated by standalone cross-check).
# Formula: LCOS = CAPEX × 0.1269 × 1000 / (365 × 0.85) × regional_mult
# Regional multipliers match standalone battery tables (same install cost geography).
# Cross-check: 4hr Med base = $295 × 0.1269 / (365 × 0.85) = $120.7/MWh. ✓
# ITC applied at evaluation time: effective_lcos = LCOS × (1 - ITC_STORAGE_RATE).
HYBRID_BATTERY_LCOS = {
    'battery4': {
...
# Map hybrid type → battery duration key for LCOS lookup
_HYBRID_BATT_KEY = {
    'solar_batt4': 'battery4', 'solar_batt8': 'battery8',
    'wind_batt4': 'battery4', 'wind_batt8': 'battery8',
}

# Map hybrid type → parent renewable type for LCOE/TX lookup
_HYBRID_PARENT_REN = {
...
def get_hybrid_tx(hybrid_type, tx_level, iso):
    """Compute transmission adder for a hybrid resource.

    Solar hybrids: parent solar TX / DC:AC ratio (AC-rating-adjusted).
    Wind hybrids: parent wind TX (no DC:AC adjustment).
    """
...
def get_hybrid_lcoe(hybrid_type, ren_level, batt_level, iso):
    """Compute component-additive LCOE for a hybrid resource ($/MWh).

    hybrid_lcoe = adjusted_renewable_lcoe + battery_lcos × (1 - ITC)

    For solar hybrids: renewable LCOE / DC:AC ratio (overbuild lowers $/MWh).
    For wind hybrids: renewable LCOE unchanged (no DC:AC overbuild).

    Args:
        hybrid_type: One of HYBRID_TYPES
        ren_level: 'Low'/'Medium'/'High' — Renewable Gen sensitivity
        batt_level: 'Low'/'Medium'/'High' — Battery sensitivity
        iso: ISO region name

    Returns:
        Total hybrid LCOE in $/MWh
    """
    parent = _HYBRID_PARENT_REN[hybrid_type]
    ren_lcoe = LCOE_TABLES[parent][ren_level][iso]

    # Solar hybrids: CF-adjusted renewable LCOE (DC overbuild lowers effective $/MWh)
    if hybrid_type in HYBRID_DC_AC_RATIOS:
        ren_lcoe = ren_lcoe / HYBRID_DC_AC_RATIOS[hybrid_type][iso]

    # Battery component with ITC discount
    batt_key = _HYBRID_BATT_KEY[hybrid_type]
    batt_lcos = HYBRID_BATTERY_LCOS[batt_key][batt_level][iso]
    batt_effective = batt_lcos * (1.0 - ITC_STORAGE_RATE)

    return ren_lcoe + batt_effective
