...
def main():
    ...
    key_thresholds = [50, 80, 90, 95, 99.9]
    ...
    steepest_list = []
    if 50.0 in pivot.columns and 99.9 in pivot.columns:
        pivot["decline"] = pivot[50.0] - pivot[99.9]
        ...
        for (scenario, fuel), row in steepest.iterrows():
            steepest_list.append({
                ...
                "lmp_at_99.9": round(float(row[99.9]), 2),
                "decline": round(float(row["decline"]), 2),
                "decline_pct": round(float(row["decline_pct"]), 1),
                "toggles": parse_scenario(scenario),
            })

        print(f"\n  Top 5 steepest LMP decline (50% -> ≥99.9%):")
        for i, s in enumerate(steepest_list[:5]):
            print(f"  {i+1}. {s['scenario']} ({s['fuel_level']}): ${s['lmp_at_50']:.2f} -> ${s['lmp_at_99.9']:.2f} (decline: ${s['decline']:.2f}, {s['decline_pct']:.1f}%)")

    # === 5. Most stable scenarios ===
    print("\n--- 5. Most stable scenarios (smallest LMP spread across thresholds) ---")

    threshold_cols_float = [c for c in pivot.columns if isinstance(c, float) and c <= 99.9]
    ...
    for (scenario, fuel), row in most_stable.iterrows():
        stable_list.append({
            ...
            "lmp_at_99.9": round(float(row[99.9]), 2) if 100.0 in row.index else None,
            "toggles": parse_scenario(scenario),
        })
