...
then performs:
  1. Consequential queue construction (dispatch-cache CO₂ accounting)
  2. Stranding analysis
  3. MAC trajectories (Scenario A: step3b pass-through, Scenario B: incremental dispatch-based)
...
This is the third stage of the split scenario pipeline:
  scenario_common.py  →  step3b_mac_queue.py (A)  →  step5_1_scenario_hourly.py (B)  →  THIS FILE

Usage:
  python step5_2a_scenario_comparison.py                # All ISOs
  python step5_2a_scenario_comparison.py --iso CAISO     # Single ISO
  python step5_2a_scenario_comparison.py --iso CAISO ERCOT  # Multiple ISOs
"""
...
def _load_step5d_queue(filename='consequential_queue_scenario_a.json',
                       mac_queue_dir=None):
    """Load pre-computed consequential queue from step3b_mac_queue output.

    Returns list of queue step dicts, or None if file not found.
    """
    if mac_queue_dir is None:
        project_root = Path(__file__).parent.parent
        mac_queue_dir = project_root / 'data' / 'step3-dispatch' / 'mac_queue'
...
def write_output(trajectories, queue_a, queue_b,
    ...
    with open(out_js, 'w') as f:
        f.write("// Auto-generated by step5_2a_scenario_comparison.py\n")
...
def main():
    ...
    mac_queue_dir = 'data/step3-dispatch/mac_queue'
    ...
    if results_a is None:
        ...
        print("  Run step3b_mac_queue.py first to produce intermediate results.")
        print("  Expected: data/step3-dispatch/mac_queue/scenario_a_{ISO}.json per ISO")
        sys.exit(1)
