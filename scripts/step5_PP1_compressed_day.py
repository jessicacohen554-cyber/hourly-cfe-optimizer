...
# Numba JIT for tight hourly dispatch loops
try:
...
_profile_cache = {}  # Cache supply profiles per ISO (they don't change between mixes)

def get_supply_profiles(iso, gen_profiles):
    """Build per-resource 8760 profiles (matches step1 exactly). Cached per ISO."""
    if iso in _profile_cache:
        return _profile_cache[iso]
    profiles = {}

    # Clean firm = nuclear seasonal-derated + geothermal flat blend — vectorized
    ...
    nuc_cfs = np.array([monthly_cf.get(m, 1.0) for m in range(1, 13)])
    blended_cfs = (nuc_share * nuc_cfs + geo_share * 1.0) / H
    # Repeat each monthly value for its number of hours
    cf_profile_arr = np.repeat(blended_cfs, month_hours)[:H]
    if len(cf_profile_arr) < H:
        cf_profile_arr = np.pad(cf_profile_arr, (0, H - len(cf_profile_arr)),
                                constant_values=1.0 / H)
    profiles['clean_firm'] = cf_profile_arr.tolist()
    ...
    # Vectorized DST-aware nighttime zeroing (replaces 365×24 Python loop)
    solar_arr = np.array(solar_raw[:H], dtype=np.float64)
    days = np.arange(365)
    is_dst = (days >= DST_START_DAY) & (days < DST_END_DAY)
    utc_offsets = np.where(is_dst, std_off - 1, std_off)
    utc_starts = (local_start + utc_offsets) % 24  # shape (365,)
    utc_ends = (local_end + utc_offsets) % 24      # shape (365,)

    # Build (365, 24) daylight mask
    hours = np.arange(24)[np.newaxis, :]            # shape (1, 24)
    us = utc_starts[:, np.newaxis]                  # shape (365, 1)
    ue = utc_ends[:, np.newaxis]                    # shape (365, 1)
    normal = us <= ue                               # shape (365, 1)
    daylight = np.where(normal,
                        (hours >= us) & (hours <= ue),
                        (hours >= us) | (hours <= ue))
    solar_arr.reshape(365, 24)[~daylight] = 0.0
    solar_raw = solar_arr.tolist()
    ...
    for rtype in ['clean_firm', 'ccs_ccgt', 'solar', 'wind', 'hydro']:
        p = profiles[rtype]
        arr = np.array(p[:H], dtype=np.float64)
        if len(arr) < H:
            arr = np.pad(arr, (0, H - len(arr)), constant_values=0.0)
        np.maximum(arr, 0.0, out=arr)
        profiles[rtype] = arr.tolist()

    _profile_cache[iso] = profiles
    return profiles


# ============================================================================
# HOURLY DISPATCH SIMULATION — Numba-accelerated inner loops
# ============================================================================

@njit(cache=True)
def _battery_dispatch_loop(total_surplus, gap, batt_power, batt_capacity,
                           batt_eff, H):
    """Daily-cycle battery charge/discharge. Modifies gap in-place."""
    ...
    for day in range(365):
        ...
        for h in range(ds, de):
            s = total_surplus[h]
            if s > 0.0 and stored < batt_capacity:
                charge = s
                if charge > batt_power:
                    charge = batt_power
                room = batt_capacity - stored
                if charge > room:
                    charge = room
                stored += charge
    ...
    return battery_matched, battery_charge_arr


@njit(cache=True)
def _ldes_dispatch_loop(total_surplus_post_batt, gap, ldes_power,
                        ldes_capacity, ldes_eff, ldes_window_hours, H):
    """Multi-day rolling-window LDES charge/discharge. Modifies gap in-place."""
    ...
    for w in range(n_windows):
        ...
        for h in range(ws, we):
            ...
            if s > 0.0 and soc < ldes_capacity:
                charge = s
                if charge > ldes_power:
                    charge = ldes_power
                room = ldes_capacity - soc
                if charge > room:
                    charge = room
    ...
    return ldes_matched, ldes_charge_arr
...
def simulate_hourly_dispatch(demand_norm, profiles, mix, procurement_pct,
    ...
    # Battery dispatch (daily cycle) — Numba-accelerated
    if battery_pct > 0:
        batt_power = (battery_pct / 100.0) * np.mean(demand)
        batt_capacity = batt_power * BATTERY_DURATION_HOURS
        battery_matched, battery_charge_arr = _battery_dispatch_loop(
            total_surplus, gap, batt_power, batt_capacity,
            BATTERY_EFFICIENCY, H)
        # Update surplus after battery charging
        total_surplus_post_batt = np.maximum(total_surplus - battery_charge_arr, 0)
    else:
        battery_matched = np.zeros(H)
        battery_charge_arr = np.zeros(H)
        total_surplus_post_batt = total_surplus.copy()

    # LDES dispatch (multi-day rolling window) — Numba-accelerated
    if ldes_pct > 0:
        ...
        ldes_matched, ldes_charge_arr = _ldes_dispatch_loop(
            total_surplus_post_batt, gap, ldes_power, ldes_capacity,
            LDES_EFFICIENCY, ldes_window_hours, H)
    else:
        ldes_matched = np.zeros(H)
        ldes_charge_arr = np.zeros(H)
