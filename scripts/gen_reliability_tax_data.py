    ...
    for f in files:
        with open(f) as fh:
            try:
                ...
                print(f"[gen] WARNING: skipping malformed {f}", file=sys.stderr)
                continue
...
def _find_run(best, iso, pathway, cost_mode=1, archetype=None, scenario='base', dg='Medium'):
    archetypes = [archetype] if archetype else ['balanced', 'nuclear-heavy', 'solar-led', 'wind-led']
    ...
    for iso in ISOS:
        ...
        for scen in ['base', 'firm_high_vre_low', 'firm_low_vre_high']:
            for dg in ['Low', 'Medium', 'High']:
                ...
                if ra:
                    c = _cost_at(ra, 99.9)
                    if c is not None: costs_a.append(c)
                if rb:
                    c = _cost_at(rb, 99.9)
                    if c is not None: costs_b.append(c)
        ra_base = _find_run(best, iso, 'A', 1, None, 'base', 'Medium')
        rb_base = _find_run(best, iso, 'B', 1, None, 'base', 'Medium')
        if ra_base: peak_gas_a = ra_base['peak_gas_mw']
        if rb_base: peak_gas_b = rb_base['peak_gas_mw']
        hero[iso] = {
            'pathway_a_median': round(median(costs_a)/1e9, 1) if costs_a else None,
            'pathway_b_median': round(median(costs_b)/1e9, 1) if costs_b else None,
            'cost_ratio': round(median(costs_b)/median(costs_a), 2) if costs_a and costs_b else None,
            'peak_gas_a_gw': round(peak_gas_a/1000, 1) if peak_gas_a else None,
            'peak_gas_b_gw': round(peak_gas_b/1000, 1) if peak_gas_b else None,
            'peak_gas_delta_gw': round((peak_gas_a - peak_gas_b)/1000, 1) if peak_gas_a and peak_gas_b else None,
            'min_cost_a': round(min(costs_a)/1e9, 1) if costs_a else None,
            'max_cost_a': round(max(costs_a)/1e9, 1) if costs_a else None,
            'min_cost_b': round(min(costs_b)/1e9, 1) if costs_b else None,
            'max_cost_b': round(max(costs_b)/1e9, 1) if costs_b else None,
        }
    return hero
                ...
                for dg in ['Low', 'Medium', 'High']:
                    ...
                    for s in run['threshold_snapshots']:
                        if s['threshold_pct'] in KEY_THRESHOLDS:
                            points.append({
                                't': s['threshold_pct'], 'year': s['year_achieved'],
                                'gas_gw': round(s['gas_need_mw']/1000, 2),
                                'peak_gw': round(s['peak_gas_mw']/1000, 2),
                                'stranded_gw': round(s['stranded_vs_peak_mw']/1000, 2),
                            })
                    if points:
                        hump[key] = {
                            'points': points,
                            'peak_gas_gw': round(run['peak_gas_mw']/1000, 1),
        ...
        for pw in PATHWAYS:
            ...
            for scen in ['base', 'firm_high_vre_low', 'firm_low_vre_high']:
                for dg in ['Low', 'Medium', 'High']:
                    ...
                    for s in run['threshold_snapshots']:
                        if s['threshold_pct'] in KEY_THRESHOLDS:
                            costs_by_t[s['threshold_pct']].append(s['cumulative_cost_usd'])
            if not costs_by_t: continue
            points = []
            for t in KEY_THRESHOLDS:
                vals = costs_by_t.get(t, [])
                if not vals: continue
                sv = sorted(vals)
                points.append({'t': t, 'min': round(sv[0]/1e9, 1), 'median': round(sv[len(sv)//2]/1e9, 1), 'max': round(sv[-1]/1e9, 1)})
            gas_share = []
            ...
            if run_base:
                for s in run_base['threshold_snapshots']:
                    if s['threshold_pct'] in KEY_THRESHOLDS:
                        ...
                        share = round(gc/total*100, 1) if total > 0 else 0
                        gas_share.append({'t': s['threshold_pct'], 'gas_cost_bn': round(gc/1e9, 1), 'gas_pct': share})
            ribbons[f"{iso}_{pw}"] = {'points': points, 'gas_share': gas_share}
    return ribbons
...
def build_resource_mix(best):
    ...
    for iso in ISOS:
        for pw in PATHWAYS:
            ...
            columns = []
            for s in run['threshold_snapshots']:
                if s['threshold_pct'] not in KEY_THRESHOLDS: continue
                mix = s['resource_mix_pct']
                mapped = {}
                total = 0
                for dk, jk in RESOURCE_MAP.items():
                    v = max(0, mix.get(dk, 0))
                    if v > 0.3:
                        mapped[jk] = v
                        total += v
                if total > 0:
                    normalized = {k: round(v/total*100, 2) for k, v in mapped.items()}
                else:
                    normalized = {}
                columns.append({'t': s['threshold_pct'], 'mix': normalized, 'raw_total_pct': round(total, 1)})
            if columns:
...
def build_mode_comparison(best):
    ...
    for iso in ISOS:
        for pw in ['A', 'B', 'C', 'D']:
            ...
            comp[f"{iso}_{pw}"] = {
                'mode1_cost_bn': round(c1/1e9, 1), 'mode2_cost_bn': round(c2/1e9, 1),
    ...
    for iso in ISOS:
        for pw in PATHWAYS:
            ...
            if c90 and c95 and c999 and c90 > 0:
                tax[f"{iso}_{pw}"] = {
                    'cost_at_90_bn': round(c90/1e9, 1), 'cost_at_95_bn': round(c95/1e9, 1),
                    'cost_at_999_bn': round(c999/1e9, 1), 'tax_ratio': round((c999-c95)/c90, 1),
                }
    return tax
...
def main():
    ...
    out_path = os.path.join(repo_root, 'dashboard', 'js', 'reliability-tax-data.js')
    if not os.path.isdir(data_dir):
        print(f"[gen] ERROR: data dir not found: {data_dir}", file=sys.stderr)
        sys.exit(1)
    ...
    for iso in ISOS:
        ...
        print(f"  {iso}: A=${h.get('pathway_a_median','-')}B  B=${h.get('pathway_b_median','-')}B  "
              f"ratio={h.get('cost_ratio','-')}  gas_delta={h.get('peak_gas_delta_gw','-')}GW")
    print(f"\n[gen] === Reliability Tax (A) ===")
    for iso in ISOS:
        t = payload['reliability_tax'].get(f"{iso}_A", {})
        print(f"  {iso}: 90%=${t.get('cost_at_90_bn','-')}B  95%=${t.get('cost_at_95_bn','-')}B  "
              f"99.9%=${t.get('cost_at_999_bn','-')}B  tax={t.get('tax_ratio','-')}x")
    print(f"\n[gen] Keys: gas_hump={len(payload['gas_hump'])}  ribbons={len(payload['cost_ribbons'])}  "
          f"mix={len(payload['resource_mix'])}  mode={len(payload['mode_comparison'])}")
    os.makedirs(os.path.dirname(out_path), exist_ok=True)
    js = f"// Auto-generated by gen_reliability_tax_data.py — do not edit\nvar RT_DATA = {json.dumps(payload, separators=(',', ':'))};\n"
    with open(out_path, 'w') as f: f.write(js)
    print(f"\n[gen] Wrote {out_path} ({os.path.getsize(out_path)/1024:.0f} KB)")
