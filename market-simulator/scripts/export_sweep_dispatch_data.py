...
def main():
    ...
    # ── Write per-ISO JSON files ──
    output_path.parent.mkdir(parents=True, exist_ok=True)
    if DASHBOARD_OUTPUT_PATH.parent.exists():
        DASHBOARD_OUTPUT_PATH.parent.mkdir(parents=True, exist_ok=True)

    for iso in isos:
        iso_output = dict(shared_meta)
        iso_output["isos"] = [iso]
        iso_output["data"] = {iso: data[iso]}
        iso_filename = f"sweep_dispatch_data_{iso}.json"
        for out_dir in [output_path.parent, DASHBOARD_OUTPUT_PATH.parent]:
            if out_dir.exists():
                iso_path = out_dir / iso_filename
                with open(iso_path, "w") as f:
                    json.dump(iso_output, f, separators=(",", ":"))
                print(f"  Per-ISO: {iso_path}")

    # ── Write combined JSON (backward compat) ──
    combined_output = dict(shared_meta)
    combined_output["isos"] = [str(i) for i in isos]
    combined_output["data"] = data

    with open(output_path, "w") as f:
        json.dump(combined_output, f, separators=(",", ":"))

    size_mb = output_path.stat().st_size / (1024 * 1024)
    print(f"Combined: {output_path} ({size_mb:.1f} MB)")
