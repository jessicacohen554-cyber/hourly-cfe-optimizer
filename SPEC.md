...
### Endogenize new-gas-build cost in argmin — IMPLEMENTATION LANDED / profile-then-sweep PENDING (Apr 19, 2026)

**Probe attempt 4 aborted (this session).** Attempted the single-turn in-proc sweep per the resume prompt. User interrupted before the `python3 scripts/_sweep_pathways_inproc.py` launch with "we need to optimize the code before trying to run again" and "it shouldn't be idling or taking this long." No runs executed, no JSONs written. The in-proc driver's own docstring claims ~0.5–1s per run after cold-start, so 12 configs should complete in ~15s of hot compute plus one cold-start per ISO (~10–15s each) — i.e., ~45–60s total. User's "idling / taking too long" signal means either (a) a prior attempt they observed showed a much longer wall-clock than the docstring advertises, or (b) they want a profile pass before committing more wall-clock. **Before the next probe launch: profile one single run (`ERCOT P3 @0.90`) under `cProfile`, emit top-20 cumtime offenders, confirm the hot path matches the docstring's cold-start-once / 0.5–1s-per-run shape, then launch the 12-config sweep.** If the profile shows an unexpected regression (e.g., per-year dispatch re-compute, per-endpoint parquet reload), fix it first — same regression likely bloats the v2 sweep 5–10×.
...
**Resume prompt for next session:** *"SPEC §24.9 endogenization is implemented (f060d38), parity-verified flag=FALSE bit-for-bit, WHOLESALE_PRICES blocker fixed (fd89c22), and the in-proc driver at `scripts/_sweep_pathways_inproc.py` is the correct shape (not `run_pathway_sweep.py`'s subprocess-per-config loop). Sanity probe STILL not executed — user flagged \"it shouldn't be idling or taking this long\" before the last launch, meaning a profile pass is now required before committing wall-clock to the full 12-config sweep. Step 1: `pip install pandas` (web container doesn't ship it). Step 2: profile ONE run first — `python3 -c \"import cProfile, pstats; from scripts._sweep_pathways_inproc import sweep_one_iso; from pathlib import Path; cProfile.run('sweep_one_iso(\\\"ERCOT\\\", Path(\\\"/tmp/probe_profile\\\"), endpoints=[0.90], pathways=[\\\"3\\\"])', '/tmp/probe.prof'); pstats.Stats('/tmp/probe.prof').sort_stats('cumulative').print_stats(25)\"` — and confirm the hot path matches the docstring's advertised shape (~0.5–1s per run after a ~10–15s ISO cold-start: numba JIT + dispatch-parquet load + EF-table load paid once). If profile shows an unexpected regression (per-year dispatch recompute, per-endpoint parquet reload, pandas-in-inner-loop), FIX IT FIRST — the same regression bloats the v2 sweep 5–10×. Step 3: once profile is clean, launch the 12-config sweep single-turn foreground (~1–1.5 min total): `for iso in ERCOT PJM; do python3 scripts/_sweep_pathways_inproc.py --iso $iso --pathways 1,3 --endpoints 0.80,0.90,0.99 --output-root /tmp/reliability_probe_sanity; done`. Driver logs per-run timing (`[ISO] [n/N] pathway@ep OK cfe=X.XX% in Ts`) and writes 12 JSONs at `/tmp/reliability_probe_sanity/<ISO>/pathway<P>_ep<NN>.json`. Step 4: extract `stranding_metadata.fleet_size_mw` from each, present the P3-vs-P1 gas-fleet delta table, wait for user approval, then launch the full 350-run v2 sweep per OPS.md. Follow-up after probe: patch `run_pathway_sweep.py:227` to call the in-proc driver instead of `subprocess.run`. Dispatch parquets NOT committed (ERCOT regenerates to ~106–110 MB, exceeds GitHub's 100 MB limit; `.gitignore:69` covers this). Audit memo `reliability_tax/AUDIT_2026-04-19_step2_3_pathway_optimizer.md` Locked-Decisions is binding. After sweep + chart-payload regen, append §24.9 to `reliability_tax/methodogy.md` — NOT as a reversal of §24.8."*
