...
(function() {
    ...
    // ========== 9. TOPOGRAPHIC CONTOUR — LIGHT (Optimization Landscape) ==========
    VARIANTS['topo'] = function() {
        // Concentric contour ellipses — light bg, colored strokes
        var cx = 680, cy = 135;
        var rings = [];
        var radii = [
            { rx: 40, ry: 24, op: 0.35, sw: 1.5, dur: 6, begin: 0 },
            { rx: 100, ry: 55, op: 0.28, sw: 1.3, dur: 6, begin: 0.7 },
            { rx: 170, ry: 85, op: 0.22, sw: 1.1, dur: 6, begin: 1.4 },
            { rx: 250, ry: 115, op: 0.18, sw: 1.0, dur: 6, begin: 2.1 },
            { rx: 340, ry: 145, op: 0.14, sw: 0.9, dur: 6, begin: 2.8 },
            { rx: 450, ry: 175, op: 0.10, sw: 0.8, dur: 6, begin: 3.5 },
            { rx: 580, ry: 210, op: 0.07, sw: 0.7, dur: 6, begin: 4.2 }
        ];
        for (var i = 0; i < radii.length; i++) {
            var r = radii[i];
            var opLow = (r.op * 0.3).toFixed(3);
            var opHigh = r.op.toFixed(3);
            // Alternate: teal for even, slate-blue for odd
            var color = (i % 2 === 0) ? 'rgba(13,148,136,' : 'rgba(71,85,105,';
            rings.push(
        ...
        // Peak marker
        var peak = [
            '<line x1="' + (cx - 8) + '" y1="' + cy + '" x2="' + (cx + 8) + '" y2="' + cy + '" stroke="rgba(13,148,136,0.45)" stroke-width="1.2"/>',
            '<line x1="' + cx + '" y1="' + (cy - 8) + '" x2="' + cx + '" y2="' + (cy + 8) + '" stroke="rgba(13,148,136,0.45)" stroke-width="1.2"/>',
            '<circle cx="' + cx + '" cy="' + cy + '" r="3" fill="rgba(13,148,136,0.30)" stroke="rgba(13,148,136,0.50)" stroke-width="1">',
        ...
        // Elevation shading
        var shading = [
            '<ellipse cx="' + cx + '" cy="' + cy + '" rx="40" ry="24" fill="rgba(13,148,136,0.06)"/>',
            '<ellipse cx="' + cx + '" cy="' + cy + '" rx="100" ry="55" fill="rgba(13,148,136,0.03)"/>'
        ].join('\n');
        // Labels
        var labels = [
            '<text x="' + (cx + 48) + '" y="' + (cy + 4) + '" font-family="monospace" font-size="7" fill="rgba(71,85,105,0.35)">optimum</text>',
            '<text x="' + (cx + 260) + '" y="' + (cy - 80) + '" font-family="monospace" font-size="7" fill="rgba(71,85,105,0.25)">cost</text>',
            '<text x="' + (cx - 300) + '" y="' + (cy + 100) + '" font-family="monospace" font-size="7" fill="rgba(71,85,105,0.25)">CFE %</text>'
        ].join('\n');
        // Light grid dots
        var gridDotsLight = [
            '<circle cx="200" cy="80" r="1.5" fill="rgba(71,85,105,0.08)"/>',
            '<circle cx="400" cy="60" r="1.8" fill="rgba(71,85,105,0.06)"/>',
            '<circle cx="1000" cy="75" r="1.5" fill="rgba(71,85,105,0.08)"/>',
            '<circle cx="1200" cy="90" r="1.8" fill="rgba(71,85,105,0.06)"/>'
        ].join('\n');
        return [shading, rings.join('\n'), peak, labels, gridDotsLight].join('\n');
    };

    // ========== 10. CONSTELLATION NETWORK — LIGHT (Interconnected Grid) ==========
    VARIANTS['constellation'] = function() {
        var nodes = [
            { x: 180, y: 65, r: 4, c: 'rgba(245,158,11,', dur: 5, begin: 0 },
            { x: 520, y: 45, r: 3.5, c: 'rgba(245,158,11,', dur: 6, begin: 1.2 },
            { x: 960, y: 55, r: 4, c: 'rgba(245,158,11,', dur: 5.5, begin: 2.5 },
            { x: 1300, y: 70, r: 3, c: 'rgba(245,158,11,', dur: 6.5, begin: 0.8 },
            { x: 100, y: 160, r: 3.5, c: 'rgba(34,197,94,', dur: 5.5, begin: 0.5 },
            { x: 380, y: 130, r: 4, c: 'rgba(34,197,94,', dur: 4.5, begin: 1.8 },
            { x: 700, y: 100, r: 4.5, c: 'rgba(34,197,94,', dur: 5, begin: 3.0 },
            { x: 1050, y: 145, r: 3.5, c: 'rgba(34,197,94,', dur: 6, begin: 0.3 },
            { x: 1350, y: 165, r: 4, c: 'rgba(34,197,94,', dur: 5.5, begin: 2.0 },
            { x: 250, y: 220, r: 4, c: 'rgba(14,165,233,', dur: 6, begin: 1.0 },
            { x: 580, y: 200, r: 3.5, c: 'rgba(14,165,233,', dur: 5, begin: 2.2 },
            { x: 850, y: 230, r: 4, c: 'rgba(14,165,233,', dur: 5.5, begin: 0.7 },
            { x: 1150, y: 210, r: 3.5, c: 'rgba(14,165,233,', dur: 6.5, begin: 3.2 },
            { x: 1400, y: 240, r: 3, c: 'rgba(14,165,233,', dur: 7, begin: 1.5 },
            { x: 60, y: 80, r: 3, c: 'rgba(99,102,241,', dur: 7, begin: 0.2 },
            { x: 450, y: 250, r: 3.5, c: 'rgba(99,102,241,', dur: 6, begin: 2.8 },
            { x: 780, y: 170, r: 4, c: 'rgba(99,102,241,', dur: 5, begin: 1.4 },
            { x: 1220, y: 120, r: 3, c: 'rgba(99,102,241,', dur: 6.5, begin: 3.5 }
        ];
        var nodesSVG = [];
        for (var i = 0; i < nodes.length; i++) {
            var n = nodes[i];
            nodesSVG.push(
                '<circle cx="' + n.x + '" cy="' + n.y + '" r="' + n.r + '" fill="' + n.c + '0.40)" stroke="' + n.c + '0.55)" stroke-width="1">' +
                '<animate attributeName="opacity" dur="' + n.dur + 's" repeatCount="indefinite"' +
                ' values="0.25;0.70;0.25" begin="' + n.begin + 's"/>' +
                '</circle>'
            );
        }
        var connections = [
            [0,5], [5,1], [1,6], [6,2], [2,7], [7,3],
            [14,4], [4,9], [9,10], [10,11], [11,12], [12,13],
            [0,4], [5,9], [1,10], [6,16], [2,17], [16,11],
            [15,10], [14,0]
        ];
        var linesSVG = [];
        for (var j = 0; j < connections.length; j++) {
            ...
            linesSVG.push(
                ...
                ' stroke="rgba(148,163,184,0.18)" stroke-width="0.8">' +
                '<animate attributeName="stroke-opacity" dur="8s" repeatCount="indefinite"' +
                ' values="0.06;0.25;0.06" begin="' + delay + 's"/>' +
                '</line>'
            );
        }
        return [linesSVG.join('\n'), nodesSVG.join('\n')].join('\n');
    };

    // ========== 11. RADIAL SUNDIAL — LIGHT (Solar/Temporal) ==========
    VARIANTS['sundial'] = function() {
        ...
        // Deterministic opacities (no Math.random for consistent rendering)
        var goldOps = [0.12, 0.16, 0.20, 0.18, 0.14, 0.10, 0.15, 0.12, 0.08, 0.06];
        var gi = 0;
        for (var i = 0; i < rayTargets.length; i++) {
            var t = rayTargets[i];
            var color = t.gold ? 'rgba(217,119,6,' : 'rgba(148,163,184,';
            var op = t.gold ? goldOps[gi++].toFixed(3) : '0.10';
            rays.push(
        ...
        var arcLen = Math.PI * arcRadius / 2;
        var dashLen = arcLen * 0.15;
        var arc = [
            '<path d="' + arcPath + '" fill="none" stroke="rgba(217,119,6,0.45)" stroke-width="2"' +
        ...
        for (var h = 0; h < 8; h++) {
            ...
            ticks.push(
                ...
                ' stroke="rgba(217,119,6,0.20)" stroke-width="1"/>'
            );
        }
        var glow = [
        ...
        var label = '<text x="' + (fx + 265) + '" y="' + (fy - 8) + '" font-family="monospace" font-size="7" fill="rgba(146,64,14,0.30)">8,760h</text>';
        return [rays.join('\n'), ticks.join('\n'), arc, glow, label].join('\n');
    };
